...
import { descriptionUpdateHandlers } from './helpers/descriptionUpdateHandlers.js';
...
router.get('/products/pending', async (req, res) => {
    ...
    res.render('vwSeller/pending', { products, stats, success_message: req.query.success_message === 'cancelled' ? 'Auction cancelled successfully!' : null });
});
...
router.post('/products/:id/cancel', async function (req, res) {
    try {
        ...
        // Multiple if -> change to tri-state if or custom error classes in model for better error handling
...
router.post('/products/:id/append-description', async function (req, res) {
    try {
        ...
        if (notifyUsers.length > 0) {
            ...
            // New style of email through helper
            const notifyData = {
                notifyUsers: Array.from(notifyMap.values()),
                product,
                description,
                productUrl
            };

            const activeHandlers = descriptionUpdateHandlers.filter(handler => handler.shouldHandle(notifyData));
            await Promise.all(activeHandlers.map(handler => handler.handle(notifyData, sendMail)));
        }
